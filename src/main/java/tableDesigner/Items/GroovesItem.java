...
import sketchDesigner.Features.Grooves;
...
public class GroovesItem extends TableDesignerItem implements DependOnMaterial {
    ...
    public GroovesItem(Material material, int type, String model, int quantity) {
        ...
        File file = new File("features_resources/grooves/icons/" + "grooves_" + type + "_icon.png");
        try {
    ...
    private static void updateMaterial(GroovesItem item) {

        GroovesItem oldGroovesItem = item;
        ...
        if (ProjectHandler.getMaterialsListInProject().contains(item.getMaterial())){
            newMaterial = oldGroovesItem.material;
        }else{
    ...
    public Map<String, ImageView> getMainImageView() {
        ...
        String imgPath = "features_resources/grooves/icons/" + "grooves_" + type + "_icon.png";
        File file = new File(imgPath);
        try {
            imagesList.put("Проточки#" + imgPath, new ImageView(new Image(file.toURI().toURL().toString())));
        } catch (MalformedURLException ex) {
            ex.printStackTrace();
        }
    ...
    public void exitEditMode() {
        if(this.editModeProperty.get()){
            GroovesItem.exitFromEditMode(this);
        }
    }
    ...
    public void updateRowPrice() {
        ...
        priceForOne = material.getGroovesTypesAndPrices().get(type - 1).doubleValue();
    ...
    private static ComboBox<GroovesType> comboBoxType;
    ...
    public static AnchorPane getAnchorPaneSettingsView() {
        if (anchorPaneSettingsView == null) {
            FXMLLoader fxmlLoader = new FXMLLoader(EdgeItem.class.getResource("/fxmls/TableDesigner/TableItems/GroovesSettings.fxml"));
    ...
    private static void settingsControlElementsInit() {
        ...
        comboBoxType = (ComboBox<GroovesType>) anchorPaneSettingsView.lookup("#comboBoxType");
        ...
        comboBoxType.setCellFactory(new Callback<ListView<GroovesType>, ListCell<GroovesType>>() {
            @Override
            public ListCell<GroovesType> call(ListView<GroovesType> param) {

                ListCell<GroovesType> cell = new ListCell<GroovesType>() {
                    @Override
                    protected void updateItem(GroovesType item, boolean empty) {
        ...
        comboBoxType.setButtonCell(new ListCell<GroovesType>() {
            @Override
            protected void updateItem(GroovesType item, boolean empty) {
                super.updateItem(item, empty);
                if (empty || item == null) {
        ...
        for (int i = 1; i <= 4; i++) {
            comboBoxType.getItems().add(new GroovesType(ProjectHandler.getDefaultMaterial(), i));
        }
        comboBoxType.getSelectionModel().select(0);
        comboBoxType.setTooltip(comboBoxType.getSelectionModel().getSelectedItem().getTooltip());

        choiceBoxModel.getItems().addAll(Grooves.getAvailableModels(comboBoxType.getSelectionModel().getSelectedItem().getType()));
    ...
    private static void enterToEditMode(GroovesItem groovesItem){
        TableDesigner.openSettings(GroovesItem.class);


        //get row data to settings
        choiceBoxMaterial.getSelectionModel().select(groovesItem.material.getReceiptName());
        for(int i =0;i<comboBoxType.getItems().size();i++){
            if(comboBoxType.getItems().get(i).getType() == groovesItem.getType()){
                comboBoxType.getSelectionModel().select(i);
                break;
            }
        }
        choiceBoxModel.getSelectionModel().select(groovesItem.model);
    ...
    public JSONObject getJsonView() {
        ...
        jsonObject.put("itemName", "GroovesItem");
    ...
    public static GroovesItem initFromJSON(JSONObject jsonObject) {
        ...
        GroovesItem groovesItem = new GroovesItem(material, type, model, quantity);
        //groovesItem.quantity = quantity;
        groovesItem.labelQuantity.setText("" + quantity);
        groovesItem.updateRowPrice();
        return groovesItem;
    }
}
...
class GroovesType {
    ...
    GroovesType(Material material, int type) {
        this.type = type;
        this.material = material;
    }
    ...
    public Node getImage() {
        File file;
        file = new File("features_resources/grooves/icons/" + "grooves_" + type + "_icon.png");
        ...
        String brief = "null";
        try {
            BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream("features_resources/grooves/brief/grooves_" + type + ".txt"), "UTF8"));
        ...
        Label label = new Label("Проточки, тип " + type);
    ...
    public Tooltip getTooltip() {
        ...
        File file;
        file = new File("features_resources/grooves/infoImages/" + "grooves_" + type + "_info_img.png");
