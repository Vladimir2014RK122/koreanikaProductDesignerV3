...
public class SketchShapeRectangleCircleCorner extends SketchShape {
    ...
    public SketchShapeRectangleCircleCorner(double layoutX, double layoutY, ElementTypes elementType, Pane sketchPane) {
        setChildShape(this);

        shapeType = ShapeType.RECTANGLE_CIRCLE_CORNER;
        ...
        //ROTATE CORNER
        Point2D pivot = new Point2D(sizeAShape - sizeRadiusShape, sizeBShape - sizeRadiusShape);
        for (int i = 1; i < 90; i++) {
...
//            System.err.println("CANT FILL RECTANGLE SHAPE");
    ...
    public SketchShapeRectangleCircleCorner(ElementTypes elementType, Material material, int depth, double sizeA, double sizeB, double radius) {
...
//        }
    ...
    private void createCutShape() {
        ...
        Polygon cutZonePolygon = null;
        {
            ...
            cutZonePolygonPoints.add(new Point2D(sizeAShape + CutDesigner.CUT_SHAPES_CUTSHIFT, sizeBShape - sizeRadiusShape));
            ...
            cutZonePolygonPoints.add(new Point2D(sizeAShape - sizeRadiusShape, sizeBShape + CutDesigner.CUT_SHAPES_CUTSHIFT));
...
//            cutShapeEdgeTop.setPadding(new Insets(0));
    ...
    private void createRadiusCutEdge() {
        ...
        pointsForEdge.add(new Point2D(sizeRadiusShape + h, 0.0));

        //ROTATE CORNER
        Point2D pivotEdge = new Point2D(0.0, 0.0);
        for (int i = 1; i < 90; i++) {
        ...
        pointsForEdge.add(new Point2D(0.0, sizeRadiusShape + h));
    ...
    private void updateCutShapeView() {
        ...
        cutShape.setSizesInfo("ШхВxR "+ (int)(sizeAShape/ Project.getCommonShapeScale()) +
                "x" + (int)(sizeBShape/ Project.getCommonShapeScale()) +
                "x" + (int)(sizeRadiusShape/ Project.getCommonShapeScale()));
    ...
    private void updateRadiusCutEdge() {
        ...
        if (sideRadiusEdge.isDefined()) {
            ...
            edgeConnectPoints.get(0).changeSetPoint(pointsForEdge.get(0));
            edgeConnectPoints.get(1).changeSetPoint(pointsForEdge.get(1));
            edgeConnectPoints.get(2).changeSetPoint(pointsForEdge.get(91));
            edgeConnectPoints.get(3).changeSetPoint(pointsForEdge.get(92));
    ...
    public void initShapeSettings() {
        ...
        //ROTATE CORNER
        Point2D pivot = new Point2D(40, 40);
        for (int i = 1; i < 90; i++) {
...
//        }
    ...
    public void initEdgesZones() {
        ...
        points.add(new Point2D(sizeAShape - sizeRadiusShape, sizeBShape - widthEdge));
...
//          triangleIconSideBEdge.setTranslateX(translateX);
//          triangleIconSideBEdge.setTranslateY(sizeBShape/2 - 2.5);
...
//           triangleIconSideCEdge.setTranslateX(translateX);
//           triangleIconSideCEdge.setTranslateY(sizeCShape/2 - 2.5);
...
//           triangleIconSideDEdge.setTranslateX(sizeDShape/2 - 2.5);
//           triangleIconSideDEdge.setTranslateY(translateY);
        ...
        translateX = (originX - (sizeAShape - sizeRadiusShape)) * Math.cos(Math.toRadians(45)) - (originY - (sizeBShape - sizeRadiusShape)) * Math.sin(Math.toRadians(45)) + (sizeAShape - sizeRadiusShape);
        translateY = (originX - (sizeAShape - sizeRadiusShape)) * Math.sin(Math.toRadians(45)) + (originY - (sizeBShape - sizeRadiusShape)) * Math.cos(Math.toRadians(45)) + (sizeBShape - sizeRadiusShape);
        ...
        if (sideRadiusEdge.isDefined()) {
            ...
            if (sideRadiusEdge instanceof Edge && ((Edge) sideRadiusEdge).isStoneHemOrLeakGroove()) {
                arcGrooveRadiusEdge = new Arc(sizeAShape - sizeRadiusShape, sizeBShape - sizeRadiusShape, sizeRadiusShape - 5, sizeRadiusShape - 5, 0, -90);
...
//            checkBoxSaveImage.setDisable(false);
    ...
    public Node getViewForListCell() {
        ...
        //ROTATE CORNER
        Point2D pivot = new Point2D((sizeAShape - sizeRadiusShape) * newScale, (sizeBShape - sizeRadiusShape) * newScale);
        for (int i = 1; i < 90; i++) {
            double originX = points.get(points.size() - 1).getX();
            double originY = points.get(points.size() - 1).getY();
            double X = (originX - pivot.getX()) * Math.cos(Math.toRadians(1)) - (originY - pivot.getY()) * Math.sin(Math.toRadians(1)) + pivot.getX();
    ...
    public static Node getStaticViewForListCell(ElementTypes elementType) {
        ...
        //ROTATE CORNER
        Point2D pivot = new Point2D(30.0, 30.0);
        for (int i = 1; i < 90; i++) {
            double originX = pointsList.get(pointsList.size() - 1).getX();
            double originY = pointsList.get(pointsList.size() - 1).getY();
            double X = (originX - pivot.getX()) * Math.cos(Math.toRadians(1)) - (originY - pivot.getY()) * Math.sin(Math.toRadians(1)) + pivot.getX();
